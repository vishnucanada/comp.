...
def main():
    ...
    rules, policy_name = GDPRPolicyParser.parse(POLICY_FILE.read_text())
    ...
    print(f"rmax={rmax}  |  tiers: critical={int(rmax*0.01)}  "
          f"high={int(rmax*0.05)}  medium={int(rmax*0.20)}")
    print("=" * 72)
    ...
    for tier, prompt in PROMPTS:
        ...
        print(f"  [g={g_used}/{rmax} ({round(100*g_used/rmax)}%)]  {text!r}")
